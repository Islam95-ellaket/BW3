...
const getAlbum = async (id) => {
    if (!id) {
        if (albumTitle) albumTitle.innerText = 'Album non trovato'
        return
    }
    ...
    try {
        ...
        const cover = album?.cover_medium || album?.cover_big || album?.cover_small || ''

        displayTracklist(album.tracks?.data || [])

        if (albumImg && cover) {
            albumImg.src = cover
            albumImg.alt = album.title || 'Copertina album'
        }

        if (artistImg && album.artist?.picture_small) {
            artistImg.src = album.artist.picture_small
        }

        if (albumType) albumType.innerText = capitalize(album.type || 'album')
        if (albumTitle) albumTitle.innerText = album.title || 'Album'
        if (albumArtist) albumArtist.innerText = album.artist?.name || 'Artista sconosciuto'
        if (albumDuration) albumDuration.innerText = formatDurationAlbum(album.duration || 0)
        if (albumRelease) albumRelease.innerText = album.release_date || 'Data non disponibile'
        if (albumLabel) albumLabel.innerText = album.label || 'Etichetta non disponibile'
        if (otherAlbumsArtist) otherAlbumsArtist.innerText = `Altro di ${album.artist?.name || 'questo artista'}`
        ...
        console.log(e)
        if (albumTitle) albumTitle.innerText = 'Album non disponibile'
    }
}
getAlbum(albumId)
